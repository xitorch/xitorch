...
from comptorch.nlfcns.rootfinder_algs.algs import lbfgs, selfconsistent, broyden, diis, gradrca
from comptorch.linfcns.solve import solve
...
class _RootFinder(torch.autograd.Function):
    ...
    def forward(ctx, fcn:Callable[[torch.Tensor],torch.Tensor],
        ...
        ctx.bck_options = set_default_option({
            # leave blank to use the default solve options
        }, bck_options)
    ...
    def backward(ctx, grad_yout:torch.Tensor):
        ...
        grad_params = torch.autograd.grad(yfcn, params_copy, grad_outputs=gyfcn,
            create_graph=torch.is_grad_enabled(), allow_unused=True)
