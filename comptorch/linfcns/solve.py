...
from comptorch.core.linop import LinearOperator
from comptorch.utils.bcast import normalize_bcast_dims
from comptorch.utils.debug import assert_runtime
from comptorch.utils.misc import set_default_option
...
def solve(A:LinearOperator, B:torch.Tensor, E:Union[torch.Tensor,None]=None,
    ...
    * bck_options: dict
    ...
    if "method" not in fwd_options:
        fwd_options["method"] = "exactsolve"

    method = fwd_options["method"].lower()
    if method == "exactsolve":
        return exactsolve(A, B, E, M)
    else:
        raise RuntimeError("Method %s is not implemented in solve" % method)
